      ...
      | _ ->
        let x::(y::_ as sstack') = sstack in
          ...
          | S _, S _ ->    (*if both operands in stack - need to move one in register*)
            ...
            | OR  -> env, [Mov (y, edx); OrBin(x, edx); Mov (L 0, edx); Setne dl; Mov (edx, y)], sstack'
            | AND -> env, [Mov (y, edx); AndBin (y, edx);               
                           ...
                           Mov (edx, y)], sstack'
            | EQUAL   -> env, [Mov (y, edx); Cmp(x, edx); Mov (L 0, edx); Sete dl; Mov (edx, y)], sstack'
            | NEQUAL  -> env, [Mov (y, edx); Cmp(x, edx); Mov (L 0, edx); Setne dl; Mov (edx, y)], sstack'
            | LESS    -> env, [Mov (y, edx); Cmp(x, edx); Mov (L 0, edx); Setl dl; Mov (edx, y)], sstack'
            | GREATER -> env, [Mov (y, edx); Cmp(x, edx); Mov (L 0, edx); Setg dl; Mov (edx, y)], sstack'
            | LEQUAL  -> env, [Mov (y, edx); Cmp(x, edx); Mov (L 0, edx); Setle dl; Mov (edx, y)], sstack'
            | GEQUAL  -> env, [Mov (y, edx); Cmp(x, edx); Mov (L 0, edx); Setge dl; Mov (edx, y)], sstack'
            )
          | _ ->           (*if one or both operands in register*)
            ...
            | OR  -> env, [OrBin(x, y); Mov (L 0, edx); Setne dl; Mov (edx, y)], sstack'
            | AND -> env, [Mov (y, edx); AndBin (y, edx);               
                           Mov (L 0, edx); Setne dl;                    
                           Mov (x, eax); AndBin (x, eax);             
                           Mov (L 0, eax); Setne al;                  
                           AndBin (eax, edx); Mov (L 0, edx); Setne dl; 
                           Mov (edx, y)], sstack'
            | EQUAL   -> env, [Cmp(x, y); Mov (L 0, edx); Sete  dl; Mov (edx, y)], sstack'
            | NEQUAL  -> env, [Cmp(x, y); Mov (L 0, edx); Setne dl; Mov (edx, y)], sstack'
            | LESS    -> env, [Cmp(x, y); Mov (L 0, edx); Setl  dl; Mov (edx, y)], sstack'
            | GREATER -> env, [Cmp(x, y); Mov (L 0, edx); Setg  dl; Mov (edx, y)], sstack'
            | LEQUAL  -> env, [Cmp(x, y); Mov (L 0, edx); Setle dl; Mov (edx, y)], sstack'
            | GEQUAL  -> env, [Cmp(x, y); Mov (L 0, edx); Setge dl; Mov (edx, y)], sstack'
            )
      in
