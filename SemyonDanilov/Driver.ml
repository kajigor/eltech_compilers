...
let parse filename = 
  ...
  read "y" |>
  read "z" |>
  (* с такой вложенностью верхушка лог. стека будет в стеке процессора *)
  (* если работа со стеком неправильная, то будет что-то вроде movl y,-4(%ebp) *)
  ("z" := !"z" * ((!"x" * (!"y" * (!"y" * (!"y" * !"y"))) + const 1))) |>
  write !"z"
